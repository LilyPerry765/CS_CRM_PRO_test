(function() {
	...
	var $Serenity_DateEditor = function(input) {
		...
		$Serenity_VX.addValidationRule(input, this.uniqueName, ss.mkdel(this, function(e1) {
		    ...
		    //console.log("addValidationRule-> input:" + value);
	...
	$Serenity_DateFormatter.format = function (value, format) {
	    ...
	    //console.log("###"+ $('html').attr('lang').toLowerCase());
		...
		if (typeof(value) === 'date') {
			...
			//console.log("Culture is :" +$('html').attr('lang').toLowerCase());
		if (($('html').attr('lang').toLowerCase() === 'fa-ir') || ($('html').attr('lang').toLowerCase() === 'fa')) {
		    var Jalalidate = Q.gregorian_to_persian(date, 'yyyy/MM/dd');
		    //console.log("Serenity_DateFormatter.format-> Input:" + date + "  Jalali Output:" + Jalalidate );
		    //return Q.htmlEncode(Jalalidate);
		    //return Q.htmlEncode(new Intl.DateTimeFormat('fa-IR').format(date));
		    return Q.htmlEncode(Q.PresianNumbers(Jalalidate, 'datetime'));
		} else
	...
	$Serenity_DecimalEditor.defaultAutoNumericOptions = function() {
		return { aDec: Q.Culture.decimalSeparator, altDec: ((Q.Culture.decimalSeparator === '.') ? ',' : '.'), aSep: ((Q.Culture.decimalSeparator === '.') ? ',' : '.'), aPad: false };
	};
	...
	$Serenity_MinuteFormatter.format = function(value,format) {
	    ...
	    //console.log("Cycle without condition");
	    return Q.htmlEncode(Q.PersiaNumber(value));
	};
	...
	$Serenity_NumberFormatter.format = function (value, format) {
		...
		if (typeof(value) === 'number') {
			...
			return Q.htmlEncode(Q.PresianNumbers(value, 'number'));
		} else if (typeof (value) === 'string') {
		    value = Q.PresianNumbers(value, 'string')
		}
	...
	var $Serenity_PersonNameEditor = function () {
	    this.$1$DisplayFormatField = null;
	};

	$Serenity_PersonNameEditor.__typeName = 'Serenity.PersonNameEditor';
	 $Serenity_PersonNameEditor.format = function(value,format) {
	    format = ss.coalesce(format, '0.##');
	   // console.log("####### PersonNameEditor ######");
		...
		setTitle: function(value) {
			if (!ss.referenceEquals(value, this.getTitle())) {
				if (ss.isNullOrUndefined(value)) {
				    ...
				    switch (value) {
				        case "Case.ActivityRequestTechnical":
				            value = "فعالیت های در دست اقدام فنی";
				            break;
				        case "Case.ActivityRequestFinancial":
				            value = "فعالیت های در دست اقدام مالی";
				            break;
				        case "Case.ActivityRequestDeny":
				            value = "فعالیت های ارجاع شده جهت اصلاح";
				            break;
				        case "Case.ActivityRequestPendding":
				            value = "فعالیت های جاری استان";
				            break;
				        case "Case.ActivityRequestLeader":
				            value = "فعالیت های استان های زیرگروه";
				            break;
				        case "Case.ActivityRequestConfirm":
				            value = "فعالیت های تایید شده";
				            break;
				        case "Case.ActivityRequestDelete":
				            value = "فعالیت های حذف شده";
				            break;
				        case "Case.ActivityRequestConfirmAdmin":
				            value = "فعالیت های تاییدشده مدیر";
				            break;
				        case "StimulReport.ActivityRequestDetail":
				            value = "گزارش ریزفعالیت استان ها";
				            break;
				        case "StimulReport.ActivityRequestReport":
				            value = "آمار";
				            break;
				        case "Messages":
				            value = "پیام ها";
				            break;
				        case "MessagesReceivers":
				            value = "پیام های دریافت شده";
				            break;
				        case "Messaging.Sent":
				            value = "پیام های ارسال شده";
				            break;
				        case "MessageLog":
				            value = "پیام کوتاه";
				            break;
	...
	ss.initClass($Serenity_DateEditor, $asm, {
	    get_value: function () {      
	      ...
	      //  console.log("get_value-> Input:" + value + "  Culture:" + culture);

			if (ss.isValue(value) && value.length === 0) {
				return null;
			}

			if ((culture === 'fa-ir') || (culture === 'fa')) {
			    var gregArray = Q.persian_to_gregorian(value);
			    return Q.formatDate(Q.parseDate(gregArray[0] + "/" + gregArray[1] + "/" + gregArray[2], 'ymd'), 'yyyy-MM-dd');
			}
			else
		...
		set_value: function (value) {
		    var culture = ($('html').attr('lang') || 'en').toLowerCase();
		    //console.log("set_value -> Input:" + value + "   Culture:" + culture );
			...
			if (ss.isNullOrUndefined(value)) {
			    ...
			    if ((culture === 'fa-ir') || (culture === 'fa')) {
			        this.element.val(Q.gregorian_to_persian(Q.parseISODateTime(value), 'yyyy/MM/dd'));
			    } else
		...
		get_sqlMinMax: function() {
			return this.get_minValue() === '1723-01-01' && this.get_maxValue() === '9999-12-31';
		},
		set_sqlMinMax: function(value) {
			if (value) {
				this.set_minValue('1723-01-01');
	...
	ss.initClass($Serenity_EntityDialog, $asm, {
		...
		getEntityTitle: function() {
		    if (!this.isEditMode()) {
		        var Title_Temp = '';
		       // console.log(this.getEntitySingular());
		        if (this.getEntitySingular() == 'Case.ActivityRequestTechnical')
		        {
		            Title_Temp = 'فعالیت در دست اقدام فنی';
		        } else if (this.getEntitySingular() == 'Messages') {
		            Title_Temp = 'پیام';
		        }
		        ...
		        } else return ss.formatString(Q.text('Controls.EntityDialog.NewRecordTitle'), Title_Temp);
			    ...
			    switch (this.getEntitySingular()) {
			        case "Case.ActivityRequestTechnical":
			            Title_Temp1 = "فعالیت در دست اقدام فنی";
			            break;
			        case "Case.ActivityRequestFinancial":
			            Title_Temp1 = "فعالیت در دست اقدام مالی";
			            break;
			        case "Case.ActivityRequestDeny":
			            Title_Temp1 = "فعالیت ارجاع شده جهت اصلاح";
			            break;
			        case "Case.ActivityRequestPendding":
			            Title_Temp1 = "فعالیت  جاری استان";
			            break;
			        case "Case.ActivityRequestLeader":
			            Title_Temp1 = "فعالیت استان های زیرگروه";
			            break;
			        case "Case.ActivityRequestConfirm":
			            Title_Temp1 = "فعالیت تایید شده";
			            break;
			        case "Case.ActivityRequestDelete":
			            Title_Temp1 = "فعالیت حذف شده";
			            break;
			        case "Case.ActivityRequestConfirmAdmin":
			            Title_Temp1 = "فعالیت تاییدشده مدیر";
			            break;
			        case "Messaging.Sent":
			            Title_Temp1 = "پیام ارسال شده";
			            break;
			        case "MessagesReceivers":
			            Title_Temp1 = "پیام دریافت شده";
			            break;
			    ...
			    } else return ss.formatString(Q.text('Controls.EntityDialog.EditRecordTitle'), Title_Temp1, (Q.isEmptyOrNull(title) ? '' : (' (' + title + ')')));
			}
		},
	...
	ss.initClass($Serenity_EntityGrid, $asm, {
		...
		getAddButtonCaption: function () {
		    ...
		    if (this.getItemName() == "Case.ActivityRequestTechnical")
		    {
		        Title_Temp = "فعالیت";
		    } else if (this.getItemName() == "Messages") {
		        Title_Temp = "پیام";
		    }
		    if (Title_Temp == '')
		    {
		        return ss.formatString(Q.text('Controls.EntityGrid.NewButton'), this.getItemName());
		    }else return ss.formatString(Q.text('Controls.EntityGrid.NewButton'),Title_Temp )//this.getItemName());
		},
		getButtons: function() {
			var self = this;
			var buttons = [];
	...
	ss.initClass($Serenity_PersonNameEditor, $asm, {
	    ...
	    format: function (ctx) {
	        return $Serenity_PersonNameEditor.format(ctx.value, this.get_displayFormat());
	    }
	}, null, [$Serenity_ISlickFormatter]);
	...
	ss.setMetadata($Serenity_PersonNameEditor, { members: [{ attr: [new Serenity.OptionAttribute()], name: 'DisplayFormat', type: 16, returnType: String, getter: { name: 'get_DisplayFormat', type: 8, sname: 'get_displayFormat', returnType: String, params: [] }, setter: { name: 'set_DisplayFormat', type: 8, sname: 'set_displayFormat', returnType: Object, params: [String] } }] });
