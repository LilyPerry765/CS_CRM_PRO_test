...
(function (Q) {
    ...
    ////////////////////////////////////////////////////////// 
    ////////////////////  Methods to handle persian date and persian numbers in UI
    function PresianNumbers(value, type) {
        ...
        //thousand seperation for big numbers. Note: You must add  annotation in model for INT64 numbers.
        if (type === 'number') {
            output = output.replace(/\,/g, '');
            var objRegex = new RegExp('(-?[0-9]+)([0-9]{3})');
            while (objRegex.test(output)) {
                output = output.replace(objRegex, '$1,$2');
            }
        }

        output = output.replace(/[0-9]/g, function (w) { return persianNumbers[+w] });
        //console.log("PersianNumbers -> Input:" + value + "  Type:" + type + "  Output:" + output);
        return output;
    }
    Q.PresianNumbers = PresianNumbers;
    ...
    function PersiaNumber(value) {
        ...
        if(Val.indexOf('-') < 0)
        {
            ...
            return chars.join('');
        }//For CycleName;Dash Between two numbers
        else {
            var Numbers = Val.split('-');

            var chars = Numbers[0].split('');
    ...
    Q.persian_to_gregorian = persian_to_gregorian;

    function gregorian_to_persian(date, format) {

        //console.log("gregorian_to_persian-> Input date:" + date);
        ...
        var jalali = jd_to_persian(gregorian_to_jd(date.getFullYear(), date.getMonth() + 1, date.getDate()));
        //console.log("gregorian_to_persian-> jalali:" + jalali);
        ...
        return format.replace(new RegExp('dd?|MM?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|fff|zz?z?|\\/', 'g'), function (fmt) {
            switch (fmt) {
                ...
                case 'ss': return pad(date.getSeconds());
                case 's': return date.getSeconds();
                case 'yyyy': return jalali[0];
                case 'yy': return jalali[0];
                case 'dd': return jalali[2];
                case 'd': return jalali[2].toString();
                case 'MM': return pad(jalali[1]);
                case 'M': return jalali[1];
                case 't': return ((date.getHours() < 12) ? 'A' : 'P');
                case 'tt': return ((date.getHours() < 12) ? 'AM' : 'PM');
                case 'fff': return Q.zeroPad(date.getMilliseconds(), 3);
                case 'zzz':
    ...
    Q.gregorian_to_persian = gregorian_to_persian;
    ...
    function parseDate(s, dateOrder) {
        ...
        if (dArray.length == 3) {
            ...
            if ((($('html').attr('lang') || 'en').toLowerCase() === 'fa') || (($('html').attr('lang') || 'en').toLowerCase() === 'fa-ir'))
                dateOrder = 'ymd';
            else
                dateOrder = dateOrder || Q.Culture.dateOrder;
            //console.log("parseDate-> input:" + s + "  dateorder:" + dateOrder + "  dArray:" + dArray);
...
(function (Q) {
    ...
    function jQueryDatepickerInitialization() {
        ...
        if (culture === 'fa') order = 'ymd';
        //console.log("jQueryDatepickerInitialization-> culture:" + culture + " order:" + order);
