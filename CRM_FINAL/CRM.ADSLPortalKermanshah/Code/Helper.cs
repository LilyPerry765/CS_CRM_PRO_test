...
namespace CRM.ADSLPortalKermanshah.Code
{
    public class Helper
    {
        ...
        public static bool AuthenticateUser(string userName, string password, out bool loginResult, out bool isActiveUser)
        {
            ...
            using (MainDataContext context = new MainDataContext())
            {
                PAPInfoUser papUser = context.PAPInfoUsers.Where(t => t.User.UserName.ToLower() == userName && t.Password == password).SingleOrDefault();
                if (papUser != null)
                {
                    if (!papUser.IsEnable)
                    {
                    ...
                    {
                        if (papUser.PAPInfo.LoginStatus != null)
                        {
