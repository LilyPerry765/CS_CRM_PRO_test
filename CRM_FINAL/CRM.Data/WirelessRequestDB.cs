...
namespace CRM.Data
{
    public static class WirelessRequestDB
    {
        public static WirelessRequestFullViewInfo GetWirelessRequestInfo(long requestId)
        {
            using (MainDataContext context = new MainDataContext())
            {
                return context.WirelessRequests
                    .Where(t => t.ID == requestId)
                    .Select(t => new WirelessRequestFullViewInfo
                    {
                        ID = t.ID,
        ...
        public static WirelessRequest GetWirelessRequestByID(long id)
        {
            using (MainDataContext context = new MainDataContext())
            {
                return context.WirelessRequests.Where(t => t.ID == id).SingleOrDefault();
            }
        }

        public static WirelessRequestInfo GetWirelessRequestInfoByTelephoneNo(long telephoneNo)
        {
            using (MainDataContext context = new MainDataContext())
            {
                return context.WirelessRequests.Where(t => t.Request.TelephoneNo == telephoneNo)
                                            .Select(t => new WirelessRequestInfo
                                            {
                                                ID = t.ID,
        ...
        public static List<WirelessRequest> GetWirelessRequestinSupportStep()
        {
            using (MainDataContext context = new MainDataContext())
            {
                return context.WirelessRequests.Where(t => t.Request.Status.RequestStep.StepTitle == "پشتیبان خارجی").ToList();
            }
        }
    }
}
