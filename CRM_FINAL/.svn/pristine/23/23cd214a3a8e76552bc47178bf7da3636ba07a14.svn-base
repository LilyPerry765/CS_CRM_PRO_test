...
join [Salas].[dbo].[Post] as AP  on AP.Post = Post.Number and Cabinet.CabinetNumber = AP.KAFO
...
where  AP.p1!= 0 and city.id=@cityid 
...
[CRM].dbo.Region ON Center.RegionID = Region.ID INNER JOIN
[CRM].dbo.City ON Region.CityID = City.ID	 
where  AP.p2!= 0 and city.id=@cityid 
...
[CRM].dbo.Region ON Center.RegionID = Region.ID INNER JOIN
[CRM].dbo.City ON Region.CityID = City.ID	 
where  AP.p3!= 0 and city.id=@cityid 
...
[CRM].dbo.Region ON Center.RegionID = Region.ID INNER JOIN
[CRM].dbo.City ON Region.CityID = City.ID	 
where  AP.p4!= 0 and city.id=@cityid 