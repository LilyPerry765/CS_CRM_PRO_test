CREATE PROCEDURE uspReportTranslationOpticalCabinetToNormalRequest
AS
BEGIN
	BEGIN TRY
		--گزارش درخواست های برگردان کافو نوری به کافو معمولی
		
		SELECT 
			ROW_NUMBER() OVER (ORDER BY R.ID) RowNumber,
			R.ID RequestId,
			...
			NP.Number NewPostNumber ,--شماره پست جدید
			...
			OVMR.VerticalRowNo OldVerticalRowNo, --طبقه قدیم
