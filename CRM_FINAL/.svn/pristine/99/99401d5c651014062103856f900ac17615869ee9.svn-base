...
namespace CRM.ADSLPortal
{
    public partial class PAPExchangeRequestForm : System.Web.UI.Page
    {
        ...
        private void ResetControls()
        {
            ...
            OldPortColumnTextBox.Text = string.Empty;
            NewPortColumnTextBox.Text = string.Empty;
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                (Context.User.Identity.IsAuthenticated))
                {
                    ...
                    //    throw new Exception("* امکان تعویض پورت ADSL به این شماره وجود ندارد !");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //if (debtAmount > 50000)
                            //    throw new Exception("امکان تعویض پورت ADSL برای شماره تلفن مورد نظر وجود ندارد، برای بررسی بیشتر به مخابرات مراجعه شود !");
                            //else
                            //{
                            ...
                            //PAPInfo papInfo = DB.SearchByPropertyName<PAPInfo>("ID", papInfoUser.PAPInfoID).SingleOrDefault();
                            //Telephone telephone = DB.SearchByPropertyName<Telephone>("TelephoneNo", telephoneNo).SingleOrDefault();

                            if (!Data.ADSLPAPPortDB.HasPAPPort(papInfo.ID, Convert.ToInt64(OldPortColumnTextBox.Text), Convert.ToInt32(CenterList.SelectedValue)))
                                throw new Exception("* پورت قدیم مورد نظر موجود نمی باشد !");
                            else
                            {
                                if (!Data.ADSLPAPPortDB.HasPAPPort(papInfo.ID, Convert.ToInt64(NewPortColumnTextBox.Text), Convert.ToInt32(CenterList.SelectedValue)))
                                    throw new Exception("* پورت جدید مورد نظر موجود نمی باشد !");
                                else
                                {
                                    ...
                                    {
                                        if (Data.ADSLPAPPortDB.GetTelephonePortNo(telephoneNo, papInfo.ID, Convert.ToInt32(CenterList.SelectedValue)) != Convert.ToInt64(OldPortColumnTextBox.Text))
                                            throw new Exception("* شماره پورت قدیم وارد شده مربوط به این تلفن نمی باشد !");
                                        else
                                        {
                                            if (!Data.ADSLPAPPortDB.GetPortStatus(papInfo.ID, Convert.ToInt64(NewPortColumnTextBox.Text), Convert.ToInt32(CenterList.SelectedValue)))
                                                throw new Exception("* پورت جدید مورد نظر خالی نمی باشد !");
                                            else
                                            {
                                                request.TelephoneNo = telephoneNo;
                                                request.RequestTypeID = (byte)DB.RequestType.ADSLExchangePAPCompany;
                                                ...
                                                request.StatusID = Data.WorkFlowDB.GetNextStatesID(DB.Action.Confirm, DB.GetStatus((byte)DB.RequestType.ADSLExchangePAPCompany, (byte)DB.RequestStatusType.Start).ID);
                                                ...
                                                papRequest.RequestTypeID = (byte)DB.RequestType.ADSLExchangePAPCompany;
                                                ...
                                                papRequest.SplitorBucht = OldPortColumnTextBox.Text; // +" - " + SpliterRowTextBox.Text + " - " + SpliterNoTextBox.Text;
                                                papRequest.NewPort = NewPortColumnTextBox.Text; // +" - " + SpliterRowTextBox.Text + " - " + SpliterNoTextBox.Text;
