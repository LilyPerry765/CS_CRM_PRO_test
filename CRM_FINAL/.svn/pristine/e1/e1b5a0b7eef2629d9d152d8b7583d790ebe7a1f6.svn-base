...
INSERT INTO [dbo].[Cabinet]
	...
	select 
		   ...
		   ,case k.TYPE_KAFO  when 'ilomam_ofak' then 3
							  ...
							  when 'llw' then 1 
							  when 'wll' then 1 
							  when 'herade  lehkad' then 3
							  when 'ERADE LEHKAD' then 3 end
           ,1
		   ,k.MARKAZI
		   ,0
		   ,0
		   ...
		   ,0
		   ,1
	from [salas].[dbo].[KAFO] as k left join CabinetType on k.MARKAZI = CabinetType.CabinetTypeName
	Go
