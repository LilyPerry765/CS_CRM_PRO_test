...
delete [CRM].dbo.RequestLog from [CRM].dbo.RequestLog as Rl join [CRM].dbo.Request as R on RL.RequestID = R.ID INNER JOIN
					 ...
					 where  City.ID = @CITY and R.RequestTypeID = 96 and R.CreatorUserID is null

delete [CRM].dbo.CutAndEstablish from [CRM].dbo.CutAndEstablish as CE join [CRM].dbo.Request as R on CE.ID = R.ID INNER JOIN
                     [CRM].dbo.Center ON R.CenterID = Center.ID INNER JOIN
                     [CRM].dbo.Region ON Center.RegionID = Region.ID INNER JOIN
                     [CRM].dbo.City ON Region.CityID = City.ID
					 where  City.ID = @CITY and R.RequestTypeID = 96 and R.CreatorUserID is null and CE.Status = 96
...
declare subscrib_cursor cursor read_only for select ID , ID_SUBSCRI ,  ID_FINANCE , C_DATE  , NewTelephone  from Salas.dbo.DCTABLE where C_DATE <> '99999999'

OPEN subscrib_cursor;
   fetch next from subscrib_cursor into @ID ,@ID_SUBSCRI, @ID_FINANCE ,	@D_C_DATE , @Telephone
while @@FETCH_STATUS = 0
	Begin
	BEGIN TRY

    set @reqeustID = CAST((CAST(STUFF( @D_C_DATE, 1, 2, '') AS nvarchar(max))+ CAST(@ID_SUBSCRI as nvarchar(max) ) + '96') as bigint)
	...
	BEGIN TRY
	set @C_DATE   = ([CRM].[dbo].[sh2mi](STUFF(STUFF(@D_C_DATE , 5 ,0,'/') , 8 , 0 ,'/')));
	END TRY
    BEGIN CATCH
    END CATCH
	if  @C_DATE is null begin set @C_DATE = CAST('1900-01-01' as smalldatetime) end
 ...
 0 --[WaitForToBeCalculate]
    ...
    INSERT INTO [CRM].[dbo].[CutAndEstablish]
     VALUES
           ...
           ,@C_DATE --<ActionCutDueDate, smalldatetime,>
           ,null --<ActionEstablishDueDate, smalldatetime,>
           ,@C_DATE --<CutDate, datetime,>
    ...
    SELECT @xml = '<?xml version="1.0" encoding="UTF-8"?>
      <Connect xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
      <TelephoneNo>' + Cast(@CenterCode as varchar) + Cast(@Telephone as varchar) + '</TelephoneNo>
      <Status>96</Status>
      </Connect>'
    ...
    INSERT INTO [CRM].[dbo].[RequestLog] VALUES
		   ...
		   update CRM.dbo.Telephone set ConnectDate = @C_DATE where TelephoneNo = @Telephone

		   	 if( (select status from CRM.dbo.Telephone where TelephoneNo = @Telephone) = 3)
               update CRM.dbo.Telephone set status = 2 where TelephoneNo = @Telephone

    -- request payment

	delete #EXPENSE
	insert into #EXPENSE select * , ROW_NUMBER() OVER(ORDER BY ID ASC) AS Row from EXPENSE where ID_SUBSCRI = @ID_SUBSCRI and (ID_EXPTYPE = 12)
	...
	if exists(select *  from EXPENSE as E1 join MALI on E1.ID_EXPTYPE = MALI.ID_EXPTYPE 
		                 ...
		                 where E1.ID = @ID_S  and E1.PAY_DATE <= MALI.STOP_DATE and E1.PAY_DATE >= MALI.START_DATE and EXPTYPE.ID in(12))
	BEGIN
	...
	INSERT INTO [CRM].[dbo].[RequestPayment]
		   ...
		   where E1.ID = @ID_S and B.KerID = -1 and EXPTYPE.ID in(12)
    END
	...
	--update T set  T.ConnectDate = (select top(1) r.InsertDate from CRM.dbo.Request as r where r.TelephoneNo = T.TelephoneNo and r.RequestTypeID = 96 order by r.InsertDate desc) from CRM.dbo.Telephone as T
